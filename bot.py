...
async def send_rent_reminders(context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"📬 Рассылка triggered at {now.strftime('%d.%m.%Y %H:%M:%S')}")
    ...
    for row in RENT_DATA[2:]:
        ...
        if day == 25:
            ...
            if total >= 0:
                continue  # нет долга — не шлем
            text_message = (
                f"Напоминаем, что по лицевому №{account_number} есть задолженность {abs(total)} р.\n"
            ...
            text_message = (
                f"⚠️ Внимание! По лицевому №{account_number} задолженность на текущий месяц увеличена.\n"
                f"Текущий баланс: {abs(total)} р.\n"
                "Оплатить можно переводом на карту Т-Банк +79062385238."
            )
        ...
        try:
            ...
            sent_count += 1
            sent_to.append(account_number)
        except Exception as e:
            print(f"❌ Ошибка отправки на {chat_id} (лицевой {account_number}): {e}")

    # Логирование
    if sent_count > 0:
        print(f"✅ Рассылка выполнена ({now.strftime('%d.%m.%Y %H:%M')}): {sent_count} сообщений отправлено.")
        print(f"📌 Лицевые счета, которым отправлено: {', '.join(sent_to)}")
    else:
